...
def long_statement_number():
    with (_tests_path / "self-provided" / "long_statement_number.sta").open() as fh:
        return fh.read()
...
def test_ASNB_tags(path_to_file, first_expected_entry_date, last_expected_entry_date):
    with _tests_path.joinpath(path_to_file).open() as fh:
        ...
        if first_expected_entry_date:
            first_expected_transaction_data["entry_date"] = first_expected_entry_date
        ...
        assert trs.transactions[2].data["amount"] == models.Amount("801.55", "D", "EUR")
        assert trs.transactions[3].data["amount"] == models.Amount("1.65", "D", "EUR")
        assert trs.transactions[4].data["amount"] == models.Amount("828.72", "C", "EUR")
        ...
        if last_expected_entry_date:
            last_expected_transaction_data["entry_date"] = last_expected_entry_date
